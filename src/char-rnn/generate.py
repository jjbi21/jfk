...
# modified source code for using LSTM and bidirectional RNN
...
def generate(decoder, corpus, prime_str='A', predict_len=100, temperature=0.8, cuda=False, model='LSTM'):
    ...
    #find end of comment character and trim anything after
    ...
    #check corpus if there was a comment identical to corpus, if so, generate another comment
    for ele in corpus:
        if(predicted == ele):
            predicted = generate(decoder, corpus, inp, predict_len, temperature, cuda)
            break
    
    #double check for removal of anything after end-of-comment character
    endOfComment = predicted.find("\v")
    endOfComment = endOfComment if endOfComment != -1 else len(predicted)
